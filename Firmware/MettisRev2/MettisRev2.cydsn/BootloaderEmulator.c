...
uint32 BootloaderEmulator_Calc8BitSum(uint32 rowNum)
{
    ...
    while (size > 0u)
    {
		...
		downSum += flashRowFromExtMem[size];
    }
    ...
    if (0u != appSizeInRows)
    {
        ...
        for (row = 0u; row < (appSizeInRows - 1u); row++)
        {
            ...
            for(i = 0u; i < CY_FLASH_SIZEOF_ROW; i++)
            {
				...
				appSum += appFlashRow[i];
            }
			
			//DBG_PRINT_TEXT("\r\nCHECKSUM ");
			//DBG_PRINT_HEX( (uint16)EMI_APP_ABS_ADDR(row) );
			DBG_PRINT_TEXT("\r\n");
			DBG_PRINT_DEC( (uint16)appSum);
...
static void BootloaderEmulator_HostLink(uint8 timeOut)
{
    ...
    do
    {
        ...
        if(ackCode == CYRET_SUCCESS)
        {
			...
			//DBG_PRINT_TEXT("*** switch: ");
			//DBG_PRINT_HEX(packetBuffer[BootloaderEmulator_CMD_ADDR]);
			//DBG_PRINT_TEXT(" ***\r\n");
            ...
            case BootloaderEmulator_COMMAND_ERASE:
                ...
                if((BootloaderEmulator_COMMUNICATION_STATE_ACTIVE == communicationState) && (pktSize >= 3u))
                {
                    ...
                    if(dataOffset == pktSize)
                    {
						...
						//DBG_PRINT_TEXT("*** program checksum: ");
						...
						//DBG_PRINT_TEXT("\t\tEMI_WriteData():\r\n");
					    //DBG_PRINT_ARRAY(dataBuffer, CY_FLASH_SIZEOF_ROW);
					    //DBG_PRINT_TEXT("\r\n");
