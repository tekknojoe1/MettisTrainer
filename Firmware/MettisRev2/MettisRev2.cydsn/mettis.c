...
static int sensor_ave[FLEX_SENSORS_NUM];


#define SENSOR_INIT 0
#define SENSOR_IDLE 1
#define SENSOR_LIFT_OFF 2
#define SENSOR_IMPACT 3
#define SENSOR_LAND 4
...
#define CADENCE_INIT 0
#define CADENCE_IDLE 1
#define CADENCE_IMPACT 2
#define CADENCE_LAND 3
#define CADENCE_AIR 4
...
void mettis_sensor_sm(int sensor_num, int sensor_value) {
	...
	switch (sensor_state[sensor_num]) {
		...
		case SENSOR_LAND:
			...
			if (sensor_value < sensor_ave[sensor_num] ) {
				sensor_delay_counter[sensor_num] = local_counter[sensor_num] / 2;
		...
		default: //SENSOR_IDLE (sensor in air)
			...
			if (sensor_value > (sensor_ave[sensor_num] * SENSOR_IMPACT_RATIO) ) {
...
void mettis_task(int sensor_peaks[], int sensor_values[]) {
	...
	switch (cadence_state) {
		...
		case CADENCE_AIR:
			...
			if (air_time_samples < AIR_TIME_MAX_SAMPLES) {
				air_time_counter++;
			}
	...
	//Heel 2 toe
	if (sensor_state[HEEL] == SENSOR_IMPACT) {
		heel2toe_counter = 0;
	}
	
	if (sensor_state[MEDIAL] == SENSOR_IMPACT) {
		heel2toe_samples = heel2toe_counter;
	}
	
	if (heel2toe_counter < HEEL2TOE_MAX_SAMPLES) {
		heel2toe_counter++;
	}
...
/*******************************************************************************
* Function Name: cadence_convert_scaled
********************************************************************************
* Summary:
...
*******************************************************************************/
void cadence_convert_scaled(uint8 scaled[], int unscaled[]) {
	int i;
...
/*******************************************************************************
* Function Name: cadence_get_cadence
********************************************************************************
* Summary:
...
*******************************************************************************/
uint8 cadence_get_cadence( void ) {
	uint32 cadence32;
	uint8 cadence8;
...
/*******************************************************************************
* Function Name: cadence_get_total_steps
********************************************************************************
* Summary:
...
*******************************************************************************/
uint32 cadence_get_total_steps( void ) {
	return total_steps;
}

/*******************************************************************************
* Function Name: cadence_get_contact_time
********************************************************************************
* Summary:
...
*******************************************************************************/
uint8 cadence_get_contact_time( void ) {
	int contact_time32;
	uint8 contact_time8;
	
	contact_time32 = contact_time_samples / CONTACT_TIME_2MS_PER_SAMPLE;
...
/*******************************************************************************
* Function Name: cadence_get_air_time
********************************************************************************
* Summary:
...
*******************************************************************************/
uint8 cadence_get_air_time( void ) {
	int air_time32;
	uint8 air_time8;
...
/*******************************************************************************
* Function Name: cadence_get_heel2toe
********************************************************************************
* Summary:
...
*******************************************************************************/
uint8 cadence_get_heel2toe( void ) {
	uint8 heel2toe_time;
...
/*******************************************************************************
* Function Name: cadence_get_impact_force
********************************************************************************
* Summary:
...
*******************************************************************************/
uint8 cadence_get_impact_force( void ) {
	int i;
	int impact_force = 0;
...
/*******************************************************************************
* Function Name: cadence_get_pronation
********************************************************************************
* Summary:
...
*******************************************************************************/
uint8 cadence_get_pronation( void ) {
...
/*******************************************************************************
* Function Name: cadence_get_cal_steps
********************************************************************************
* Summary:
...
*******************************************************************************/
uint8 cadence_get_cal_steps( void ) {
	uint8 steps;
