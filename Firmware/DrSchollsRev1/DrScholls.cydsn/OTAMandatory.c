...
cystatus EMI_WriteData(uint32 dataAddr, uint32 dataSize, uint8 *data)
{
	...
	winbond_ota_write_buf(dataAddr, data, dataSize);
        ...
        if(ackCode == CYRET_SUCCESS)
        {
            ...
            ***************************************************************************/
            case BootloaderEmulator_COMMAND_EXIT:
				...
				EMI_ReadData(EMI_MD_BASE_ADDR, CY_FLASH_SIZEOF_ROW, metadata);
				DBG_PRINTF("Meta data back out:\r\n");
				DBG_PRINT_ARRAY(metadata, CY_FLASH_SIZEOF_ROW);
				DBG_PRINTF("\r\n");
